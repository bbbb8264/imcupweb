...
<?php
	...
	if(isset($_GET['announceid'])){
		$sql = "select id,DATE_FORMAT(publishtime, '%Y年%m月%d號%h:%i') datetime,title,content from basketballannounce where id='".$_GET['announceid']."'";
		...
		$dir = 'basketballannouncedata/'.$_GET['announceid'];
		if(is_dir($dir)){
		...
		$sql = "select id,DATE_FORMAT(publishtime, '%Y年%m月%d號') date,DATE_FORMAT(publishtime,'%h:%i') time,title from basketballannounce order by publishtime desc";
		$result = $conn->query($sql);
