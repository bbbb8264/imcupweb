...
<body>
	...
	if(isset($_GET['announceid'])){
		$sql = "select title,content from basketballannounce where id='".$_GET['announceid']."'";
		...
		echo '<div id="currentpage">
				...
				<form class="ui form" action="addannounce.php" method="post">
					...
					<div id="filelist">';
		if(is_dir('basketballannouncedata/'.$_GET['announceid'])){
			if($handle = opendir('basketballannouncedata/'.$_GET['announceid'])){
		...
		$sql = "select id,DATE_FORMAT(publishtime, '%Y年%m月%d號%h:%i') datetime,title from basketballannounce order by publishtime desc";
		$result = $conn->query($sql);
